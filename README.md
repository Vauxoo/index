[![Build Status](https://git.vauxoo.com/vauxoo/index/badges/14.0/pipeline.svg)](https://git.vauxoo.com/vauxoo/index/pipelines)
[![Coverage Report](https://git.vauxoo.com/vauxoo/index/badges/14.0/coverage.svg)](https://coverage.vauxoo.com/14-0-index)
[![Runbot Status](http://runbot.vauxoo.com/runbot/badge/109/14.0.svg)](http://runbot.vauxoo.com/runbot/repo/git-git-vauxoo-com-vauxoo-index-git-109)
...
1. Install travis2docker in order to a create local environment (docker
   container) copy of last changes on the index repository:
    ...
    of index for make functional tests or even for do new developments:
...
3. Now we can use the t2d command. This will be accomplish by using the next
   command line:
    ...
    ```bash
    $ t2d git@git.vauxoo.com:vauxoo/index.git 14.0
    ```

    The result after run this t2d script will be a log that have a list of
    local paths, in index case, will have a list of 2 paths

    ```bash
    ['${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_index.git/14.0/3.8/env_1_job_1',
     '${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_index.git/14.0/3.8/env_2_job_1']
    ```
...
4. The next step is to run the last mentioned scripts, we highly recommend to
   add the next extra arguments.
    ...
    ```bash
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_index.git/14.0/3.8/env_2_job_1/10-build.sh --no-cache
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_index.git/14.0/3.8/env_2_job_1/20-run.sh --entrypoint=bash
    ```
...
5. After the last script is finish you will notice that your prompt has change,
   ...
   like this:
    ...
    ```bash
    [odoo@a458896bf8f5]~/build/vauxoo/index$
    (14.0) $
    ```
    ...
    index:
        ...
        - main repository will be ALWAYS inside /root/build/vauxoo/index
